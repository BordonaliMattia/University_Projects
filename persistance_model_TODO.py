...
from utils import process_dad_data, build_random_LS_VS, point_scores, plot_point_forecasts, plot_point_metrics, scale_data
...
pred = np.roll(df_VS_targets.values,70)
 
plt.plot(df_VS_targets.values.reshape(df_VS_targets.shape[0] * output_dim))
plt.plot(pred.reshape(df_VS_targets.shape[0] * output_dim))
plt.show()

df_predictions = pred.reshape((VS_days,output_dim)) # TODO
df_predictions = pd.DataFrame(data=pred, index=df_VS_targets.index, columns=df_VS_targets.columns).sort_index()
