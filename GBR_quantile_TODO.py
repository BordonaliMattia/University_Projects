...
for quantile in q_set:
    ...
    # TODO
    model[quantile] = GradientBoostingRegressor(loss="quantile", alpha=quantile, max_depth = max_depth, n_estimators = n_estimators, learning_rate = learning_rate)
    model[quantile].fit(df_LS_scaled, df_LS_targets_reshaped)
    model[quantile].score(df_LS_scaled, df_LS_targets_reshaped)
    # Fit model
    # TODO
...
predictions = dict()
for quantile in q_set:
    predictions[quantile] = model[quantile].predict(df_VS_scaled)# TODO
    plt.plot(predictions[quantile])
